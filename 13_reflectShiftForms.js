...
function reflectShiftForms() {
  ...
  const filtered = data
    .map((row, i) => ({
      id: row[COLUMN_ID - COLUMN_START],
      name: row[COLUMN_NAME - COLUMN_START],
      order: i,
      submit: row[COLUMN_SUBMIT - COLUMN_START],
      check: row[COLUMN_CHECK - COLUMN_START],
      reflect: row[COLUMN_REFLECT - COLUMN_START],
    }))
    .filter(({ submit, check, reflect }) =>
  ...
  targetSheets.forEach(dailySheet => {
    ...
    filtered.forEach(({ id, name, order }) => {
      ...
      // その日程のデータを取得
      const rowData = personalSheet.getRange(dateRow, 1, 1, FORM_COLUMN_NOTE).getValues()[0];
      ...
      dailySheet.getRange(SHIFT_ROW_START_TIME, personalCol, 2, 1).setValues(timeArray.map(v => [v]));

      // 開始時間と終了時間を日付に紐づけて取得
      const base = new Date(date.getFullYear(), date.getMonth(), date.getDate());
      const startTime = normalizeTimeToDate(base, start) || new Date(base.getFullYear(), base.getMonth(), base.getDate(), DEFAULT_OPEN_HOUR, DEFAULT_OPEN_MINUTE);
      const endTime = normalizeTimeToDate(base, end) || new Date(base.getFullYear(), base.getMonth(), base.getDate(), DEFAULT_CLOSE_HOUR, DEFAULT_CLOSE_MINUTE);

      // 各時間帯において、
      timeList.forEach((t, ti) => {
        // セルの時間帯を取得
        const [h, m] = t.split(":" ).map(Number);
        const cellTime = new Date(base.getFullYear(), base.getMonth(), base.getDate(), h, m);
      ...
      // 背景色を一括で反映
      dailySheet.getRange(SHIFT_ROW_START, personalCol, timeList.length, 1).setBackgrounds(bgArray.map(c => [c]));
    });
  ...
  filtered.forEach(({ order }) => {
    manageSheet.getRange(order + ROW_START, COLUMN_REFLECT).setValue(REFLECT_TRUE);
  });
