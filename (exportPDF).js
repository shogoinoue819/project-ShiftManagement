...
function exportPDF() {
  ...
  // 日付形式のシートだけ抽出
  const targetSheets = allSheets.filter(sheet => /^\d{1,2}\/\d{1,2}$/.test(sheet.getName()));
  ...
  const startDate = `${THIS_YEAR}/${String(smonth).padStart(2, '0')}/${String(sday).padStart(2, '0')}`;

  // 最後のシートの日付を取得し、ファイル名に使う
  const endSheetName = targetSheets[targetSheets.length - 1].getName();
  const [emonth, eday] = endSheetName.split('/');
  const endDate = `${THIS_YEAR}/${String(emonth).padStart(2, '0')}/${String(eday).padStart(2, '0')}`;

  // ファイルを作成
  const newSpreadsheet = SpreadsheetApp.create(`シフト_${startDate}~${endDate}`);
  ...
  targetSheets.forEach(sheet => {
    ...
    // シフト希望メモを非表示
    copiedSheet.hideRows(SHIFT_ROW_START_TIME, SHIFT_ROW_NOTE - SHIFT_ROW_START_TIME + 1);
    // シフト希望反映背景色をクリア
    ...
    copiedSheet.setRowHeights(SHIFT_ROW_MEMBERS, SHIFT_ROW_END - SHIFT_ROW_MEMBERS + 1, newHeight);
  });
  ...
  // PDFファイルを作成し、ファイリング
  const pdfBlob = response.getBlob().setName(`シフト_${startDate}~${endDate}.pdf`);
