...
function shareShifts() {
  ...
  // シフト日程のデータを取得
  const data = manageSheet.getRange(MANAGE_DATE_ROW_START, MANAGE_DATE_COLUMN, getLastRowInCol(manageSheet, MANAGE_DATE_COLUMN) - MANAGE_DATE_ROW_START + 1, MANAGE_SHARE_COLUMN - MANAGE_DATE_COLUMN + 1).getValues();
  ...
  data.forEach((row, i) => {
    ...
    if (isComplete === true && isShare === SHARE_FALSE) {
      ...
      try {
        ...
        pdfSheet.setRowHeights(SHIFT_ROW_MEMBERS, SHIFT_ROW_END - SHIFT_ROW_MEMBERS + 1, newHeight);
  ...
  if (sharedCount > 0) {
    ...
    ui.alert(`✅ 完成済みのシフトを ${sharedCount} 日分、共有ファイルに反映しました！`);
...
function shareOnlyOneShift() {
  ...
  // 「共有済」に更新
  manageSheet.getRange(getOrderByDate(dateStr) + MANAGE_DATE_ROW_START, MANAGE_SHARE_COLUMN).setValue(SHARE_TRUE);
