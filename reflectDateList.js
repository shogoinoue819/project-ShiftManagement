...
function reflectDateList() {
  ...
  // A列に日付をセット
  targetSheet.getRange(FORM_ROW_START, FORM_COLUMN_DATE, numDates, 1).setValues(dateList);

  // B列（完了チェック）を FALSE で初期化
  const falseValues = Array(numDates).fill([false]);
  manageSheet.getRange(MANAGE_DATE_ROW_START, MANAGE_COMPLETE_COLUMN, numDates, 1).setValues(falseValues);

  // C列（共有ステータス）を "未共有" で初期化
  const shareValues = Array(numDates).fill([`${SHARE_FALSE}`]);
  manageSheet.getRange(MANAGE_DATE_ROW_START, MANAGE_SHARE_COLUMN, numDates, 1).setValues(shareValues);
  ...
  if (deleteStart <= maxRow) {
    ...
    Logger.log("⚠️ 削除対象の行がシート範囲外だったため、削除をスキップしました");
  }

  Logger.log(`✅ 日程 ${numDates} 件をテンプレートに反映し、完了・共有列を初期化しました`);
}
